...
_USER_CONFIG_DIR = Path.home() / ".vplatform"
...
def _read_yaml_root(config_path: Path) -> Path | None:
    ...
    try:
        ...
        return None
    for key in ("root", "vplatform_root"):
...
def resolve_vplatform_root(start: Path | None = None, explicit: Path | str | None = None) -> Path:
    """配置发现链：explicit → VPLATFORM_ROOT → cwd 向上 → ~/.vplatform → 包开发根。"""
    if explicit:
        root = Path(explicit).expanduser().resolve()
        logger.debug("使用显式 root={}", root)
        return root

    if env_root := os.getenv("VPLATFORM_ROOT"):
        root = Path(env_root).expanduser().resolve()
        logger.debug("使用 VPLATFORM_ROOT={}", root)
        return root
    ...
    if _USER_CONFIG_PATH.exists():
        if home_root := _read_yaml_root(_USER_CONFIG_PATH):
            logger.debug("从 ~/.vplatform/config.yaml 读取 root={}", home_root)
            return home_root
        logger.debug("使用 ~/.vplatform 作为 root")
        return _USER_CONFIG_DIR
...
def resolve_config_path(root: Path | None = None) -> tuple[Path, Path]:
    """返回 (vplatform_root, config_yaml_path)。"""
    resolved_root = resolve_vplatform_root(explicit=root)
...
def find_config_example(root: Path | None = None) -> Path | None:
    resolved = resolve_vplatform_root(explicit=root)
    for candidate in (
